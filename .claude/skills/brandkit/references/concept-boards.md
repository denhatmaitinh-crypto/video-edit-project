...
Render all options in editable HTML with `${HF_WORKFLOWS}/brandkit/scripts/brandkit.py preview`, then publish them per `inline-widgets.md` (board PNG screenshots shown inline + editable HTML links). Do not generate image-model duplicates and do not send filenames alone.
...
Load `inline-widgets.md`. Screenshot and upload every returned board in the same sandbox batch, show each option's confirmed PNG inline, and provide the confirmed HTML URL as the editable download. For 2–3 options, stack the complete option blocks vertically in one message.
...
When interaction is allowed, send a normal message asking the user to review and comment. In explicit auto/no-question mode, do not ask a follow-up question. Do not self-select, treat your own visual assessment as approval, or start typography before the user responds.
...
- Render the real brand name and short sample copy through `${HF_WORKFLOWS}/brandkit/scripts/brandkit.py preview`.
...
Render a combined board only when all three slots are approved and the user requested a full identity/Brandkit or asks to see them together. Otherwise show the available individual review boards. Use `${HF_WORKFLOWS}/brandkit/scripts/brandkit.py preview` with one `stage: "essential"` review containing the approved logo, palette, and type slots.
